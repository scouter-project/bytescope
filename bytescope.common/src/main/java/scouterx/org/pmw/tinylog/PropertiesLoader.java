...
package scouterx.org.pmw.tinylog;
...
import scouterx.org.pmw.tinylog.labelers.Labeler;
import scouterx.org.pmw.tinylog.policies.Policy;
import scouterx.org.pmw.tinylog.writers.Writer;

/**
 * Loads configuration for {@link Logger Logger} from properties.
 */
final class PropertiesLoader {
	...
	private static Writer readWriter(final Properties properties, final String propertyName, final String writerName) {
		for (Class<?> implementation : findImplementations(Writer.class)) {
			scouterx.org.pmw.tinylog.writers.PropertiesSupport propertiesSupport = implementation.getAnnotation(scouterx.org.pmw.tinylog.writers.PropertiesSupport.class);
	...
	private static Writer loadWriter(final Properties properties, final String propertiesPrefix, final scouterx.org.pmw.tinylog.writers.Property[] definition,
	...
	private static Object[] loadParameters(final Properties properties, final String propertiesPrefix, final scouterx.org.pmw.tinylog.writers.Property[] definition) {
	...
	private static Labeler parseLabeler(final String string) {
		...
		for (Class<?> implementation : findImplementations(Labeler.class)) {
			scouterx.org.pmw.tinylog.labelers.PropertiesSupport propertiesSupport = implementation.getAnnotation(scouterx.org.pmw.tinylog.labelers.PropertiesSupport.class);
			if (propertiesSupport != null) {
	...
	private static Policy parsePolicy(final String string) {
		...
		for (Class<?> implementation : findImplementations(Policy.class)) {
			scouterx.org.pmw.tinylog.policies.PropertiesSupport propertiesSupport = implementation.getAnnotation(scouterx.org.pmw.tinylog.policies.PropertiesSupport.class);
			if (propertiesSupport != null) {
				if (name.equalsIgnoreCase(propertiesSupport.name())) {
